...
import { runScenario, wasAccepted, finalMatchesToolNumber, calledToolSuccessfully, issuedCorrection } from "../../src/eval/harness.js";
...
describe("buildProbeAgent", () => {
  it("accepts an optional logger and runs the full turn", async () => {
    ...
    const logger: Logger = { log: (level, event, fields) => void records.push({ level, event, fields }) };
  ...
  it("runs without a logger (noop default)", async () => {
    const scenario: Scenario = {
      ...
      assertions: [issuedCorrection(), calledToolSuccessfully("disk_free"), wasAccepted(), finalMatchesToolNumber("disk_free")],
    };
