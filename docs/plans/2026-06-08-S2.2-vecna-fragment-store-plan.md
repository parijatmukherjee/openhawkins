# S2.2 — VECNA Fragment Store (remember / recall / reinforce) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Stand up `@openhawkins/memory` (VECNA) — a decay-aware memory store over embedded SQLite where the runtime can `remember` a fragment, `recall` the most relevant fragments for a query (FTS5 text relevance blended with importance·decay, tag overlap, taint penalty, and tendril bias), and `reinforce` a fragment that proved useful.

**Architecture:** A new workspace package `@openhawkins/memory` that depends on `@openhawkins/state` (for the `SqlDriver`, `openDatabase`, and the `migrate` runner) and `@openhawkins/core` (for the Gate's `Provenance`/`Trust` types). A `fragments` table plus an FTS5 mirror kept in sync by triggers. Recall is a two-step: an FTS5 `MATCH` selects candidates with their `bm25` scores, then a **pure** scoring function ranks them by a blend of text relevance, importance·decay (computed at query time from an injected clock), tag overlap, taint penalty, and tendril bonus. No embeddings yet — that's S2.3.

**Tech Stack:** TypeScript (strict) · `node:sqlite`/`bun:sqlite` FTS5 + bm25 (verified available) · Vitest · the existing >99% coverage gate.

**Spec:** [`docs/specs/2026-06-08-S2-state-memory-design.md`](../specs/2026-06-08-S2-state-memory-design.md) — milestone **S2.2** (§4 layout, §8 VECNA store). Decisions §12.2 (lexical-now recall path is the always-available baseline), §12.4 (decay at query time, injected clock).
...
packages/memory/                       # VECNA — decay-aware memory
  package.json                         # @openhawkins/memory (deps: core + state)
...
### Task 1: Scaffold `@openhawkins/memory`
...
Expected: completes; `node_modules/@openhawkins/memory` symlink exists; `package-lock.json` updated. Commit the lockfile.

Note: `npm run build` will FAIL after this step because the barrel imports `fragment.js`/`schema.js`/`recall.js`/`store.js` which don't exist until Tasks 2–5. That is expected for scaffolding — do not stub them. Verify only that `npm install` created the symlink: `ls -la node_modules/@openhawkins/` shows `memory -> ../../packages/memory`.
...
git commit -m "chore(memory): scaffold @openhawkins/memory package + project references"
```
...
Run: `npx tsc -p packages/core/tsconfig.json` is not needed; instead confirm `Trust` is exported by core. Run: `node -e "import('@openhawkins/core').then(m => console.log(typeof m))"` after `npm run build` of core — but core may not rebuild here. Simplest: grep that `Trust` is exported.
...
git commit -m "feat(memory): pure recall scoring (bm25 + decay + tags + taint + tendril)"
```
...
- **Spec §8.1 — the `Fragment` shape (id/text/tendril/tags/importance/trust/taint/timestamps/uses):** Task 2. ✓
...
- **Spec §8.3 — decay at query time (0.5^(age/halfLife), 7-day half-life), blended score (bm25 + importance·decay + tag overlap − taint penalty + tendril bonus), `reinforce` bumps importance (clamped ≤1) + uses + lastUsedAt:** Tasks 3 (pure scorer + decay), 5 (reinforce SQL clamps via `MIN(1.0, …)`). ✓
- **Spec §4 — package layout `core ← state ← memory`, memory depends on state's driver/migrate:** Task 1 (deps + references), Task 5 (imports `openDatabase`/`migrate` from `@openhawkins/state`). ✓
